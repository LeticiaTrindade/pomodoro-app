  ...
  return (
    ...
    >
      ...
      <header className={`text-center transition-all ${theme === 'dark' ? 'text-slate-300' : ''}`}>
        <h1 className={`text-3xl md:text-5xl font-bold bg-gradient-to-r from-accent to-accent-fg bg-clip-text mb-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'
          }`}>
          Pomodoro Timer ⏰
        </h1>
        <p className={`text-sm md:text-base transition-colors ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'
          }`}>
          Foque, pause, repita.
        </p>
      </header>
      ...
      {/* Card Principal */}
      <section className={`w-full max-w-md md:max-w-lg lg:max-w-2xl p-8 md:p-12 rounded-3xl shadow-2xl border flex flex-col items-center gap-8 transition-all duration-300 ${theme === 'dark'
          ? 'bg-slate-800/80 backdrop-blur-xl border-slate-700/50 shadow-black/20'
